...
- **Theme config update is text-based**: `set_theme_name_in_config` in `theme.py` uses regex/text manipulation to update `rockgarden.toml` rather than a proper TOML writer (stdlib `tomllib` is read-only). Works for typical configs but could mangle unusual formatting. Swap for `tomli_w` or similar before 1.0.
